...
#############draw mahtplot#############
######################################################
   gwas.data <- read.table("snp.gwas.KC.p0.0001.txt",sep="\t",header=TRUE)
   p <- gwas.data$p
...
#set some variables for mhtplot.
   pngfile.names <- paste("Fig2b.KC_snp_GWAS_p0.0001_B73_v4.png",sep="")
   title.name <- "Kernel Color SNP GWAS"
