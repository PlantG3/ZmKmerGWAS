...
exp = read.csv("all_KC_p_2.326453e-08_kmer_add_gene_exp.csv", stringsAsFactors = F,header = FALSE)
head(exp)
...
net_P12M10 = blockwiseModules(datexp, power = 7,
...
TOM = TOMsimilarityFromExpr(datexp, power = 7)
# Export the network into edge and node list files Cytoscape can read
cyt = exportNetworkToCytoscape(TOM,
                               edgeFile = paste("YW_new_p_P7M200_Cyt-edges_p0.2.txt", sep=""),
                               nodeFile = paste("YW_new_p_P7M200_Cyt-nodes_p0.2.txt", sep=""),
                               weighted = TRUE,
                               threshold = 0.2,
...
write.table(out_module,file="YW_new_p_P7M200_modules.txt",sep="\t",quote=FALSE,row.names = FALSE)