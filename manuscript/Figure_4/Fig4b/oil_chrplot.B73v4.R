...
genome <- "B73"
trait <- "1a3o_oil"
chr.size <- read.delim("B73Ref4.length", header=F, stringsAsFactors=F)
...
# plot
##################################################################################################
pdffile <- paste0(trait, ".kmerGWAS.", genome, ".sigplot.new.pdf")
...
plot(NULL, NULL, ylim = yrange,
     ...
     main = "Associated loci in B73", cex.main=2, cex.lab = 2,
     bty = "n")
